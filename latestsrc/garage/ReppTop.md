...
ReppPet for preliminary documentation). Please do not make
...
PetInput page for background). Both functions take optional
...
so-called YY format; see the PetInput page); :sppp (an
LKB-internal format, see the LkbSppp page); and :raw (a list
...
(see the LogonTop page), for example, includes the following
...
if need be), in this case the file sample.yy. See PetInput
...
support in the PET parser (see the ReppPet page for details)
...
SafFspp page), albeit at the expense of backwards
