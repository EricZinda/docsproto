...
(PTB). The selection of sentences included in DeepBank is
...
DeepBank is derivative of the PTB.

DeepBank annotations are distributed through the
...
DeepBank is appropriately acknowledged and its license terms
preserved.

For communication with DeepBank developers and users, there
...
analysis at all in DeepBank.

# Corpus Organization

DeepBank follows the section division familiar from the PTB,
...
The not manually validated, robust analyses in DeepBank are
...
The native representation of the HPSG analyses in DeepBank
...
(ItsdbDerivations); (e) a simplified phrase
...
DeepBank annotations are combined in the files below
